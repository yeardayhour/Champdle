  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-center w-full">
      <div class="flex-shrink w-full relative">
        ...
        />
        <ul
          v-if="!isCloseAutocomplete && fuzzyChampionNameMap.length > 0"
        ...
        >
          <li
            v-for="(name_item, index) in fuzzyChampionNameMap"
...
async function guess() {
  ...
  error_message.value = ""
  try {
    const eng_name = state.translateChampionName(trimmed_name, true)
    if (eng_name === undefined) {
      error_message.value = "error-no-such-champion"
      name.value = ""
      return
    }
...
const fuzzyChampionNameMap = computed(() => {
  const text = name.value.trim().normalize("NFD")
  if (text.length < 1) {
    return []
  }
  const searcher = nameSearcher(text)
  const result: LocalName[] = _.keys(state.api_data.champion_local_name_map)
    .filter((local_name) => {
      return searcher.test(local_name.normalize("NFD"))
    })
    .map((local_name) => {
      const english_name = state.translateChampionName(local_name, true)
      return {
        english_name: english_name,
        local_name: state.translateChampionName(english_name, false), // to use proper-cased name
      }
    })
  return result
})
